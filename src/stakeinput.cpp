...
// The zRWT block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zRWT is a hash of the serial
...
{
    //Create an output returning the zRWT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZRWTOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zRWT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zRWT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZRWTOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zRWT output", __func__);
...
{
    CzRWTTracker* zrwtTracker = pwallet->zrwtTracker.get();
    CMintMeta meta;
    if (!zrwtTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zrwtTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a RWT stake is the outpoint
