...
CzRWTTracker::CzRWTTracker(std::string strWalletFile)
{
...
CzRWTTracker::~CzRWTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRWTTracker::Init()
{
...
bool CzRWTTracker::Archive(CMintMeta& meta)
{
...
bool CzRWTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRWTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRWTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRWTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzRWTTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzRWTTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzRWTTracker::GetSerialHashes()
{
...
CAmount CzRWTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRWTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRWTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRWTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRWTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRWTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRWTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRWTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRWTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRWTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRWTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRWTWallet* zRWTWallet)
{
    bool iszRWTWalletInitialized = (NULL != zRWTWallet);
    CMintMeta meta;
    ...
    if (! iszRWTWalletInitialized)
        zRWTWallet = new CzRWTWallet(strWalletFile);
    meta.isSeedCorrect = zRWTWallet->CheckSeed(dMint);
    if (! iszRWTWalletInitialized)
        delete zRWTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRWTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRWTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRWTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRWTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRWTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRWTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRWTWallet* zRWTWallet = new CzRWTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zRWTWallet);
        }
        delete zRWTWallet;
        LogPrint("zero", "%s: added %d dzrwt from DB\n", __func__, listDeterministicDB.size());
    }
