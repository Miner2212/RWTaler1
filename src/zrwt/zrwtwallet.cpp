...
CzRWTWallet::CzRWTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrwt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRWTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRWTSeed_deprecated()) {
                    LogPrintf("%s: Updated zRWT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrwt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrwt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRWTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRWTCount(nCountLastUsed);
    else if (!walletdb.ReadZRWTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRWTWallet::Lock()
{
    seedMaster = 0;
}

void CzRWTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRWTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRWT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRWTWallet::LoadMintPoolFromDB()
{
...
void CzRWTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRWTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRWTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrwtTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRWTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRWT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrwtTracker which also adds to database
    pwalletMain->zrwtTracker->Add(dMint, true);
...
void CzRWTWallet::SeedToZRWT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRWTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRWTWallet::GenerateDeterministicZRWT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRWTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRWTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRWTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
