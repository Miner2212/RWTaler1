...
public:
    ...
    bool WriteZRWTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRWTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRWTSeed_deprecated(uint256& seed);
    bool EraseZRWTSeed();
    bool EraseZRWTSeed_deprecated();

    bool WriteZRWTCount(const uint32_t& nCount);
    bool ReadZRWTCount(uint32_t& nCount);
