...
/**
 * RWTaler RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZrwtSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzrwtseed(const UniValue& params, bool fHelp);
extern UniValue getzrwtseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzrwt(const UniValue& params, bool fHelp);
extern UniValue dzrwtstate(const UniValue& params, bool fHelp);
