...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 RWT.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzRWTReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzRWTReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzRWTReset.connect(boost::bind(NotifyzRWTReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzRWTReset.disconnect(boost::bind(NotifyzRWTReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zrwtTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
