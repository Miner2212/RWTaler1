...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZRWTModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zrwtTracker->HasMintTx(hash))) {
            //zRWT stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZRWT;
...
bool IsZRWTType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZRWT:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZRWT || type == TransactionRecord::MNReward) {
