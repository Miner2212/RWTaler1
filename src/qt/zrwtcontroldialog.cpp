...
#include "ui_zrwtcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zrwtTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zrwtTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRWT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRWT seed used to mint this zRWT is not the same as currently hold in the wallet";
            else
